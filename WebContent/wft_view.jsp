...
%>
Workflow template id: <%=wftId%> | <a href='/cflow/wft_start.jsp?wftid=<%=wftId%>&token=<%=token%>'>Test</a> | <a href='/cflow/wft_edit.jsp?wftid=<%=wftId%>&token=<%=token%>'>Edit</a> | <a href="/cflow/wft.jsp?token=<%=token%>">All</a> 
...
<script language="javascript">
	...
	if ( hasProductInstall && !hasRequestedVersion ) {
		...
		designerContent=AC_FL_RunContent(
			...
			"FlashVars", "token=<%=token%>&devid=<%=devid%>&wizard=true&mode=<%=mode%>&usemode=2&subAction=viewWft&MMredirectURL="+MMredirectURL+'&MMplayerType='+MMPlayerType+'&MMdoctitle='+MMdoctitle+"",
			"width", "800",
			"height", "540",
			"align", "middle",
			"id", "CFlowWftDesigner",
		...
		designerContent=AC_FL_RunContent(
				...
				"FlashVars", "token=<%=token%>&devid=<%=devid%>&wizard=true&mode=<%=mode%>&usemode=2&subAction=viewWft&usrname=Steve%20Liu",
				"width", "800",
				"height", "540",
				"align", "middle",
				"id", "CFlowWftDesigner",
	...
	<!--
		function viewWft(){
			...
			if(designerIsReady == false){
				setTimeout(function(){viewWft()}, 500);
			}else{
				CFlowWftDesigner.ExtLoadWft("<%=token%>", "<%=wftId%>");
			}
		}
