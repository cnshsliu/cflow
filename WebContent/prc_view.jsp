...
%>
Workflow process id: <%=prcId%> | <a href='/cflow/prc_edit.jsp?prcid=<%=prcId%>&startby=<%=startby%>&token=<%=token%>'>toggle to edit mode</a>
	...
	<!--
		function viewProcess(token, prcid, startBy, wftid){
			...
			if(designerIsReady == false){
				...
				CFlowWftDesigner.ExtViewProc(token, prcid, startBy, wftid);
			}
		}
		...
		function CancelProc(prcid){
			var req = new Request({
				  url: '/cflow/rest/process/status?action=cancelProcess&prcid=' + prcid + "&token=<%=token%>",
					method: 'put',
					emulation: false,
		...
		function SuspendProc(prcid){
			var req = new Request({
				  url: '/cflow/rest/process/status?action=suspendProcess&prcid=' + prcid + "&token=<%=token%>",
				 	method: 'put',
					emulation: false,
					onSuccess: function(json){  },
					onFailure: function(){ alert("failed."); }
		...
		function ResumeProc(prcid){
			var req = new Request({
				  url: '/cflow/rest/process/status?action=cancelProcess&prcid=' + prcid + "&token=<%=token%>",
				  method: 'put',
					emulation: false,
					onSuccess: function(json){ alert("canceled"); },
					onFailure: function(){ alert("failed."); }
				  });
				  req.send();
		}
		function SuspendTask(prcid, nodeid){
			var req = new Request({
			  url: '/cflow/rest/process/status?action=suspendWork&prcid=' + prcid + "&nodeid=" + nodeid + "&token=<%=token%>",
			  method: 'put',
				emulation: false,
				onSuccess: function(json){  },
				onFailure: function(){ alert("failed."); }
		...
		function ResumeTask(prcid, nodeid){
			var req = new Request({
				  url: '/cflow/rest/process/status?action=resumeWork&prcid=' + prcid + "&nodeid=" + nodeid + "&token=<%=token%>",
				  method: 'put',
					emulation: false,
					onSuccess: function(json){ },
					onFailure: function(){ alert("failed."); }
