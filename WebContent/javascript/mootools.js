...
var typeOf = this.typeOf = function(item){
	...
	if (item.name){
		...
		if (item.nodeType == 3) return (/\S/).test(item.value) ? 'textnode' : 'whitespace';
...
local.isXML = function(document){
	...
	(document.nodeType === 9 && document.documentElement.name !== 'HTML');
};
...
local.setDocument = function(document){
	...
	if (this.isHTMLDocument){
		...
		try {
			...
			starSelectsClosed = (selected && selected.length && selected[0].name.charAt(0) == '/');
		} catch(e){};
		...
		if (testNode.querySelectorAll) try {
			...
			this.starSelectsClosedQSA = (selected && selected.length && selected[0].name.charAt(0) == '/');
		} catch(e){};
...
local.matchSelector = function(node, tag, id, classes, attributes, pseudos){
	if (tag){
		if (tag == '*'){
			if (node.name < '@') return false; // Fix for comment nodes and closed nodes
		} else {
	...
	' ': function(node, tag, id, classes, attributes, pseudos, classList){ // all child nodes, any level
		...
		if (this.isHTMLDocument){
			getById: if (id){
				...
				if ((!item && node.all) || (this.idGetsName && item && item.getAttributeNode('id').value != id)){
					// all[id] returns all the elements with that name or id inside node
					// if theres just one it will return the element, else it will be a collection
					children = node.all[id];
					if (!children) return;
					if (!children[0]) children = [children];
					for (i = 0; item = children[i++];) if (item.getAttributeNode('id').value == id){
...
var pseudos = {
	...
	'first-of-type': function(node){
		var name = node.name;
		while ((node = node.previousSibling)) if (node.name === name) return false;
		return true;
	},

	'last-of-type': function(node){
		var name = node.name;
		while ((node = node.nextSibling)) if (node.name === name) return false;
		return true;
	},

	'only-of-type': function(node){
		var prev = node, name = node.name;
		while ((prev = prev.previousSibling)) if (prev.name === name) return false;
		var next = node;
		while ((next = next.nextSibling)) if (next.name === name) return false;
		return true;
	},
...
local.getAttribute = function(node, name){
	...
	// getAttribute is faster than getAttributeNode().value
...
local.override(/./, function(expression, found, first){ //querySelectorAll override
	...
	if (local.starSelectsClosedQSA) for (i = 0; node = nodes[i++];){
		if (node.name > '@' && (!hasOthers || !local.uniques[local.getUIDHTML(node)])) found.push(node);
...
local.override(/^#[\w-]+$/, function(expression, found, first){ // ID override
	...
	if (!el) return found;
	if (local.idGetsName && el.getAttributeNode('id').value != id) return false;
...
var camels = ['defaultValue', 'tokenString', 'cellPadding', 'cellSpacing', 'colSpan', 'frameBorder', 'maxLength', 'readOnly',
